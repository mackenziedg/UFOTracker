...
## Breweries Per State  
```{r}
beer.path <- "/Users/shane/Documents/UFOTracker/data/raw/brew_count_by_state_1984_2017.csv"
sightings.path <- "/Users/shane/Documents/UFOTracker/data/raw/ufo_sightings.csv"
...
```{r}
all_florida <- read.csv(file = "/Users/shane/Documents/UFOTracker/data/raw/all_florida.csv", header = TRUE)
all_florida <- filter(all_florida, year >= 1998)

internet_rurality <- read.csv(file = "/Users/shane/Documents/UFOTracker/data/raw/internet_by_rurality.csv", header = TRUE)
...
```{r}
allpossreg <- regsubsets(sightings_year ~ per_capita_gdp_current + breweries + num_movies, nbest=6, data=all_florida)
...
With the limited scope of our understanding of statistics, we are unable to compare all 5 explanatory factors with the predicted variable, sightings per year. As we learn more advanced statistical techniques, we can come back to this analysis and improve on modelling with spatial and time dependencies.  
