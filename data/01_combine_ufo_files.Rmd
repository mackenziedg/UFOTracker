...
statepops3.raw <- fread(file.path(my.directory, statepops3.path), header = TRUE)
...
```{r}
yrs <- as.year(1974:2016)

statepops.db3[1,]

# filter_(statepops.db3, Name)

# colnames(statepops.db3) <- statepops.db3[1,]
# colnames(statepops.db3)

# View(zoo(statepops.db3))
  
# View(statepops3.raw)
zd <- zoo(select(statepops3.raw, -year), statepops3.raw$year)
zi <- zoo(NA, as.year(yrs))

z <- merge(zd, zi)
z[-zi,]


interp <- na.approx(z[,"Alabama"])
