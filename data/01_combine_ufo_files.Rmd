...
beer.db <- beer.raw %>% 
  ...
  gather_("year", "breweries", as.character(seq(1984, 2016))) %>% 
...
sightings.db2 <- sightings.db %>% 
  ...
  mutate((num_sightings/population) * 1000)
  #select(-n)
...
Join files together
```{r}
all.states.normalized.ßdf <- sightings.db2 %>% 
...
Step 4: Write out
```{r}
write_csv(all.states.df, "raw/all_states_normalized.csv")
```
